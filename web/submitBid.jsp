<%
    ...
    try {
         ...
         */
        String query = "SELECT A.MinimumBid, A.CurrentHiBidder, A.CurrentHiBid, A.BidIncrement FROM Auction A WHERE A.AuctionID = ?";
        ...
        if (rs.next()) {
            ...
            minimumBid = rs.getDouble("MinimumBid");
        }
        rs.close();
        if (maximumBid < minimumBid) {
            ...
            double bidPrice = 0.0;
            if (currentHiBidder.equals("")) {
                bidPrice = Math.min(maximumBid, currentHiBid); //fine
            } else {
                bidPrice = Math.min(maximumBid, currentHiBid + bidIncrement);
            }
            ...
            if (maximumBid > currentHiBid && maximumBid <= auctionMaxBid) {
                ...
                ps.setInt(2, auctionId);
                ps.setDouble(3, Math.max(bidPrice + bidIncrement, auctionMaxBid)); 
                ...
                conn.commit();
                    ...
                    ps = conn.prepareStatement(query3);
                    ps.setString(1, maxBidder);
                    ps.setDouble(2,  Math.min(bidPrice + bidIncrement, auctionMaxBid));
                    ps.setInt(3, auctionId);
                    ps.executeUpdate();
                    conn.commit();
            ...
            session.putValue("auctionId", auctionId);
